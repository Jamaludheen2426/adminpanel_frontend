...
export default function EmailCampaignsPage() {
  ...
  const openEditDialog = (campaign: EmailCampaign) => {
    ...
    setForm({
      ...
      status: campaign.status === "completed" || campaign.status === "paused" ? "draft" : campaign.status,
    });
    setDialogOpen(true);
  };
              ...
              <TableBody>
                {campaigns.map((campaign) => (
                  <TableRow key={campaign.id}>
                      ...
                      <div>
                        ...
                        {campaign.holiday_name && (
                          <p className="text-xs text-muted-foreground">
                            {holidays?.find((h) => h.key === campaign.holiday_name)?.name}
                          </p>
                        )}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge variant={statusColors[campaign.status] as "default" | "secondary" | "outline"}>
                        {campaign.status}
                      </Badge>
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="text-xs">
                        <span className="text-green-600">{campaign.total_sent}</span>
                        {" / "}
                        <span className="text-red-600">{campaign.total_failed}</span>
                      </div>
                    </TableCell>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value: "holiday" | "scheduled" | "recurring") =>
            ...
            {form.campaign_type === "holiday" && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm({ ...form, holiday_name: value })}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setForm({
                      ...form,
                      email_config_id: value === "default" ? undefined : parseInt(value),
                    })
                  }
                >
                  <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value: "all_users" | "active_users" | "verified_users" | "custom") =>
            ...
            <div className="p-3 bg-muted rounded-md">
              ...
              <div className="flex flex-wrap gap-2">
                {(() => {
                  ...
                  const systemVars = variableMappings ? Object.keys(variableMappings) : [];
                  const allVars = [...new Set([...templateVars, ...systemVars])];
                  return allVars.map((variable) => (
                    <Badge
                      key={variable}
                      variant={templateVars.includes(variable) ? "default" : "outline"}
                      title={variableMappings?.[variable] ? `Source: ${variableMappings[variable].source}` : "Template variable"}
                    >
                      {`{{${variable}}}`}
                    </Badge>
                  ));
              ...
              <div className="text-xs text-muted-foreground mt-2 inline-flex items-center gap-1">
                <Badge variant="default" className="text-[10px] px-1">colored</Badge> = from template,
                <Badge variant="outline" className="text-[10px] px-1">outlined</Badge> = system variables
              </div>
            </div>
          </div>
